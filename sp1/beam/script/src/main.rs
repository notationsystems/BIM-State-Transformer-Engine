...
use gat_sp1_beam_lib::{
    ...
    NUMERIC_PROFILE_ID, PROOF_TYPE, RECEIPT_FORMAT, REQUEST_FORMAT, SCHEMA_VERSION, SP1_VERSION,
};
...
fn read_request(path: &Path) -> Result<CheckedRequest> {
    ...
    if request.sp1_version != SP1_VERSION || request.transition_event_seq == 0 {
...
fn validate_public_values(proof: &SP1ProofWithPublicValues, expected: &[u8]) -> Result<()> {
    if proof.sp1_version != SP1_VERSION {
        bail!("proof was generated by a different SP1 version");
    }
